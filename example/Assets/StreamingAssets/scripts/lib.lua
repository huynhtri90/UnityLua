local tbTree = Import("scripts/tree.lua");

Tree = tbTree.PrintTree;

local tbList = Import("scripts/list.lua");
CreateList = tbList.new;

-- 按照统一的格式回调函数
function CallBack(tbCallBack)
...
function ShowStack(szErrMsg)
	Log("LuaError: " .. szErrMsg);
	Log(debug.traceback());
end

-- 获得一个32位数中指定位段(0~31)所表示的整数
function LoadBits(nInt32, nBegin, nEnd)
	if (nBegin > nEnd) then
		local _ = nBegin;
		nBegin = nEnd;
		nEnd   = _;
...
function SetBits(nInt32, nBits, nBegin, nEnd)
	if (nBegin > nEnd) then
...
function GetLocalDayTime(nUtcSec)
...
--	1970年3月 返回2
--	……依此类推
function GetLocalMonth(nUtcSec)
...
--		2012-09-28
--		2012.09.28
function ParseDatetime(szDatetime)
...
-- 返回值:	中文数字
function Transfer4LenDigit2CnNum(nDigit)	
...
function LiangBarskyAlgorithm(l,t,w,h, x1,y1,x2,y2)
...
function LeftTimeToString(nTime) -- 剩余时间，解决 os.date("%H:%M:%S", cd) 的时区问题
	if nTime <= 0 then
		return "00:00";
	end
...
-- 模式替换tab数据中的某些列
function TabSub(tab, tbSubSetting)
...
function ParsePlistPath(fileName)
	return string.match(fileName, "(.+%.plist)/(.+)");
end

-- 去除指定字符串首尾指定字符
function StrTrim(szDes, szTrimChar)
...
function MergeTableDiff(src, diff)
...
function ReadOnlyTB(table)
	local result = {};
	for key, value in pairs(table) do
		if type(value) == "table" then
			result[key] = ReadOnlyTB(value);
		end
	end

	local mt = { __index = table, __newindex = _NewIndexReadonly};
	setmetatable(result, mt)
	return result
end

function _NewIndexReadonly(table, key, value)
	Log("Attempt to modify read-only table", key);
